...
const COMPANIES = [
  'All', 'Google', 'Amazon', 'Microsoft', 'TCS', 'Infosys', 'Wipro',
...
function InterviewQuestions() {
  ...
  const [stats, setStats] = useState({ total: 0, companies: COMPANIES.length - 1 });

  // 1. Fetch Questions with Callback
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setStats({
          total: res.data.count || res.data.questions?.length || 0,
  ...
  // 2. Debounce Search and Filter Requests (300ms)
  useEffect(() => {
  ...
  const generateAIQuestions = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setAiError('Failed to generate AI questions.');
      }
    } catch (err) {
      console.error(err);
      setAiError('Error generating questions. Please try again.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleUpvote = async (id) => {
    ...
    setQuestions((prevQuestions) =>
      prevQuestions.map((q) => (q.id === id ? { ...q, upvotes: (q.upvotes || 0) + 1 } : q))
    );

    try {
      await axios.post(`${API}/api/jobs/upvote-question`, { questionId: id, userId: user?.id });
    } catch (err) {
      console.error('Upvote failed:', err);
      // Rollback on failure
      setUpvoted((prev) => ({ ...prev, [id]: false }));
      setQuestions((prevQuestions) =>
        prevQuestions.map((q) => (q.id === id ? { ...q, upvotes: Math.max(0, (q.upvotes || 1) - 1) } : q))
      );
    }
  };

  const handleResetFilters = () => {
    setFilters({ company: 'All', role: 'All', category: 'All', difficulty: 'All', search: '' });
  };
      ...
      <div style={styles.container}>
        ...
        <div style={styles.header}>
          ...
          <p style={styles.subtitle}>
            Real interview questions from top companies. Filter by company, role, and difficulty. Prepare smarter, get
            hired faster.
          </p>
          <div style={styles.headerStats}>
            <div style={styles.headerStat}>
              <span style={styles.headerStatNum}>{stats.total}+</span>
              <span style={styles.headerStatLabel}>Questions</span>
            </div>
            <div style={styles.headerStat}>
              <span style={styles.headerStatNum}>{stats.companies}</span>
          ...
          {showAiPanel && (
            <div style={styles.aiPanelBody}>
              ...
              <div style={styles.aiFormRow}>
                ...
                <button style={styles.aiGenBtn} onClick={generateAIQuestions} disabled={aiLoading}>
                ...
                <div style={styles.aiQuestions}>
                  ...
                  {aiQuestions.map((q, i) => (
                    <div key={i} style={styles.aiQuestionCard}>
                      ...
                      {q.hint && <p style={styles.aiHint}>💡 {q.hint}</p>}
                    </div>
                  ))}
                </div>
              )}
                  ...
                  <div style={styles.qExpanded}>
                    {q.tags && q.tags.length > 0 && (
                      <div style={styles.qTags}>
                        {q.tags.map((tag) => (
                          <span key={tag} style={styles.qTag}>
                            #{tag}
                          </span>
                        ))}
                      </div>
...
const styles = {
  ...
  aiQText: { flex: 1, fontSize: '14px', color: '#1e293b', lineHeight: '1.5', margin: 0 },
  aiHint: { fontSize: '12px', color: '#64748b', marginTop: '8px', fontStyle: 'italic', margin: 0 },
  ...
  qText: { fontSize: '15px', color: '#0f172a', lineHeight: '1.6', cursor: 'pointer', fontWeight: '500', marginBottom: '12px', marginTop: 0 },
  ...
  qHints: {},
  qHintTitle: { fontSize: '13px', fontWeight: '600', color: '#0f172a', marginBottom: '8px', margin: 0 },
  qHintList: { paddingLeft: '16px', margin: '8px 0 0 0' },
